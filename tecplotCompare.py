...
with open(file_path_main, 'r') as mainFile, open(file_path_ref, 'r') as refFile, open(new_file_path, 'w') as modified:
    ...
    modified_second_line = variables_line.strip() + ',      "rho_dif",      "Ma_dif,      "Pa_dif",      "T_dif"'
    ...
    for line1, line2 in zip(mainFile, refFile):
        ...
        if stripped_lineMain.startswith("ZONE"):
            ...
            rho_dif = 1
            ma_dif = 2
            modified_line = '\t'.join(mainValues + ["{:.6E}".format(ratio_Rho_percent)] + ["{:.6E}".format(ratio_Ma_percent)] + ["{:.6E}".format(ratio_pressure_percent)] + ["{:.6E}".format(ratio_T_percent)]) + '\n'
